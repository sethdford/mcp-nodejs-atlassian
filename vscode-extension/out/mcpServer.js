...
class McpServerManager {
    ...
    async runOAuthSetup() {
        try {
            ...
            const useWebSetup = await vscode.window.showQuickPick(['Web-based setup (Recommended)', 'Terminal-based setup'], {
    ...
    async runWebOAuthSetup() {
        ...
        try {
            ...
            server.listen(port, 'localhost', () => {
                ...
                vscode.window.showInformationMessage('OAuth authorization page opened in your browser. Complete the authorization and return to VS Code.', 'Show Output').then(selection => {
    ...
    async exchangeOAuthCode(clientId, clientSecret, code, redirectUri) {
        try {
            ...
            this.outputChannel.show();
            vscode.window.showInformationMessage('OAuth setup completed successfully! Check the output panel for your configuration variables.', 'Show Output', 'Open Settings').then(selection => {
                if (selection === 'Show Output') {
                    this.outputChannel.show();
                }
    ...
    async runTerminalOAuthSetup(serverPath) {
        ...
        this.outputChannel.show();
        vscode.window.showInformationMessage('OAuth setup wizard is running in the background. Check the output panel for progress and follow the instructions.', 'Show Output').then(selection => {
            if (selection === 'Show Output') {
                this.outputChannel.show();
            }
        });
